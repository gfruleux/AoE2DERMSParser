...
class MainFrame:
    ...
    def update_map_size(self, map_size_key: str):
        map_size = MAP_SIZE_DICT[map_size_key]
        self._ptr_lbl_map_size.config(text=map_size)
        self.adapter.update_map_size(map_size)

    def update_map_resources(self, map_res_key: str):
        map_resources = MAP_RESOURCES_DICT[map_res_key]
        self._ptr_lbl_map_resources.config(text=map_resources)
        self.adapter.update_map_resources(map_resources)

    def update_game_type(self, game_type_key: str):
        game_type = GAME_TYPE_DICT[game_type_key]
        self._ptr_lbl_game_type.config(text=game_type)
        self.adapter.update_game_type(game_type)
    ...
    def draw(self):
        ...
        combo_map_res.current(3)
        row_index += 1
        lbl_map_res_txt = MAP_RESOURCES_IDX[3]
        ...
        row_index += 1
        lbl_map_size_txt = MAP_SIZE_IDX[3]
        ...
        combo_gamme_type = ttk.Combobox(self.root, values=list(GAME_TYPE_DICT.keys()), justify="center",
        ...
        row_index += 1
        lbl_game_type_txt = GAME_TYPE_IDX[0]

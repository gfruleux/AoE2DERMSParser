...
class MainFrame:
    def __init__(self, runner, path_gen, path_out, path_rms, map_size, map_type, map_resources):
        self.runner = runner
        self.root = tk.Tk()
        ...
        self._map_resources: int or None = map_resources
    ...
    def draw(self):
        ...
        def update_map_resources(ptr_label, map_res_key: str):
            self._map_resources = MAP_RESOURCES_DICT[map_res_key]
            ptr_label.config(text=self._map_resources)
        ...
        combo_map_res.bind("<<ComboboxSelected>>",
                           lambda event: update_map_resources(lbl_map_res, var_map_res.get()))
        ...
        if self._map_resources is not None:
            lbl_map_res_txt = self._map_resources
            combo_map_res.current(MAP_RESOURCES_IDX.index(self._map_resources))
        else:
            lbl_map_res_txt = "Not selected"
        row_index += 1
        ...
        if self._map_size is not None:
            ...
            combo_map_size.current(MAP_SIZE_IDX.index(self._map_size))
        else:
        ...
        if self._map_type is not None:
            ...
            combo_map_type.current(MAP_TYPE_IDX.index(self._map_type))
        else:
